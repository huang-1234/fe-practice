 ...
 */
{
  ...
  */
  ...
  let str21: string = anything; // 不会提示错误
}
{
  /* 2. unknown
...
}
{
  ...
  let undeclared31: undefined = undefined; // 鸡肋
  ...
  let undeclared32: undefined = undefined;
...
}
{
  ...
  const userInfo33: { id?: number; name?: null | string } = {
    id: 1,
    name: "Captain",
  };
  ...
  userInfo33.id?.toFixed(); // Optional Chain
  const myName = userInfo33.name ?? `my name is ${userInfo33.name}`; // 空值合并
}
/* 4. never
